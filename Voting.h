...
//GLOBALS 
struct currentValidID currentValidID; // to store current Valid user ID parameters
CANDIDATE candidateArray[20]; // to store information all candidates
int numberOfCandidates; // the total number of candidates standing for voting
...
// to check whether the global branch code and inputed branch code is matching
...
void initiateNewVoting()
{
    printf("\nNew Voting Initiation:\n");
    
    printf("\nVoting for which Year: ");
    scanf("%d",&currentValidID.year);
    printf("Enter branch code (5 letters):");
    scanf("%s",currentValidID.branch);
    printf("Enter voter population:");
   ...
   for (int i = 0; i < currentValidID.totalVoters; i++)
    ...
    for (int i = 0;i < numberOfCandidates; i++)
    {
        ...
        printf("Enter name of Candidates %d: ",i+1);
...
void saveVotingInfoInFile(){
    printf("Saving Voting Info in File...\n");
    FILE *fp = fopen("VotingInfo.txt", "w");
    if(fp==NULL)
    {
...
void loadVotingInfoFromFile()
{
    FILE *f1,*f2,*f3;
    f1=fopen("VotingInfo.txt","r");
    if(f1==NULL)
        printf("Not Exist");
...
void adminPanel()
{
    while(1){
        ...
        if(authenticateAdmin()!=1){
            printf("\n Invalid Username or Password, check ReadMe.md\n");            
            break;
        }

        printf("\n\nLOGGED IN SUCCESSFULLY (Enter to continue)");
		getch();
        ...
        while(1)
        {
            ...
            switch(input)
            {
                case '1':
                    initiateNewVoting();
                    saveVotingInfoInFile();
                    createCandidateFiles();
                    break;
                case '2':
                    loadVotingInfoFromFile();
                    break;
                case '3':
                    WinnerCid = getWinner();
                    if(WinnerCid != -1){
...
void studentPanel()
{
    ...
    while(1)
	{
        ...
        if(isValid(userID)!=1)
        {
            printf("\n  Invalid User ID (Enter to continu~e, follow the format in Readme.md)");
            getch();
            continue;
        }
